...
namespace Manifest.Views
{
    public partial class RoutinePage : ContentPage
    {
        ...
        public RoutinePage()
        {
            InitializeComponent();
            try{
                Debug.WriteLine("SIGNED IN WITH APPLE: USERID: " + (string)Application.Current.Properties["userId"]);
                setting = false;
        ...
        private async void CreateList()
        {
            ...
            for (int i = 0; i < todaysRoutines.Count; i++)
            {
                if (todaysRoutines[i].Id == routineId)
                {
                    ...
                    //    showParticles.IsVisible = true;
                    //    showParticles.IsRunning = true;
                    //    showParticles.IsActive = true;
                    ...
                    if (toAdd.NumSubOccurances != 0)
                    {
                        ...
                        foreach (SubOccurance subTask in subTasks)
                        {
                            ...
                            if(subTask.instructions.Count == 0)
                            {
                                subTask.IsSublistAvailable = false;
                            }
                            else
                            {
                                subTask.IsSublistAvailable = true;
                            }
                        ...
                        var routineHasStarted = IsRoutineStarted(subTaskSouce);

                        if (routineHasStarted == false)
                        {
                            showParticles.IsVisible = false;
                            showParticles.IsRunning = false;
                            showParticles.IsActive = false;

                            parent.IsComplete = false;
                            parent.IsInProgress = false;
                            routineIcon.Source = previousPicUrl;

                            string res = await RdsConnect.updateOccurance(parent, false, false, AppConstants.BaseUrl + AppConstants.updateGoalAndRoutine);
                        }
                        else
                        {
                            var routineIsInProgress = IsRoutineInProgress(subTaskSouce);
                            ...
                            if (routineIsInProgress)
                            {
                                ...
                                string res = await RdsConnect.updateOccurance(parent, true, false, AppConstants.BaseUrl + AppConstants.updateGoalAndRoutine);
                            }
                            else
                            {
                                showParticles.IsVisible = true;
                                ...
                                string res = await RdsConnect.updateOccurance(parent, false, true, AppConstants.BaseUrl + AppConstants.updateGoalAndRoutine);
                            }
                        }
        ...
        bool IsRoutineStarted(ObservableCollection<SubOccurance> list)
        {
            var result = false;

            foreach(SubOccurance task in list)
            {
                if(!(task.IsComplete == false && task.IsInProgress == false))
                {
                    result = true;
                    break;
                }
        ...
        bool IsRoutineInProgress(ObservableCollection<SubOccurance> list)
        {
            var result = false;

            var isRoutineStated = IsRoutineStarted(list);
            var isRoutineCompleted = IsRoutineComplete(list);

            if(isRoutineStated == true && isRoutineCompleted == false)
            {
                result = true;
            }
        ...
        bool IsRoutineComplete(ObservableCollection<SubOccurance> list)
        {
            var result = false;
            ...
            foreach (SubOccurance task in list)
            {
                if (task.IsComplete == true && task.IsInProgress == false)
                {
                    totalCompletedTasks++;
                }
            }

            if(totalCompletedTasks == list.Count)
            {
                result = true;
            }
